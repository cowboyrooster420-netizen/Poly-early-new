...
export class OiCalculationService {
  ...
  private async calculateLiquidityImpact(
    ...
    if (!liquidity || liquidity.availableLiquidity <= 0) {
      if (thresholds.fallbackToOiCalculation) {
        logger.debug({ marketId, tradeSide }, 'No liquidity data available, falling back to OI calculation');
        return this.calculateOiImpact(tradeUsdValue, openInterest, true);
      }
      throw new Error(`No liquidity data available for market ${marketId}`);
    }

    const impactPercentage = (tradeUsdValue / liquidity.availableLiquidity) * 100;
  ...
  private async calculateVolumeImpact(
    ...
    if (!volumeData || volumeData.volumeNh <= 0) {
      if (thresholds.fallbackToOiCalculation) {
        logger.debug({ marketId }, 'No volume data available, falling back to OI calculation');
        return this.calculateOiImpact(tradeUsdValue, openInterest, true);
      }
  ...
  private calculateOiImpact(
    ...
    if (openInterest <= 0) {
      return {
        ...
        threshold: isFallback ? thresholds.fallbackOiPercentage : thresholds.minOiPercentage,
        details: { 
          tradeValue: tradeUsdValue, 
          openInterest, 
    ...
    const threshold = isFallback ? thresholds.fallbackOiPercentage : thresholds.minOiPercentage;
  ...
  private async getAvailableLiquidity(
    ...
    try {
      ...
      if (error instanceof AxiosError) {
        ...
        logger.warn({ error, marketId }, 'Unexpected error fetching orderbook data');
      }
      return null;
    }
  }
  ...
  private calculateLiquidityFromOrderbook(
    ...
    const availableLiquidity = tradeSide === 'buy' ? askLiquidity : bidLiquidity;
  ...
  private async getRecentVolume(marketId: string): Promise<VolumeData | null> {
    ...
    try {
      ...
      if (!response.data?.length) {
        logger.debug({ marketId }, 'No recent trades found for volume calculation');
      ...
      const volume24h = thresholds.volumeLookbackHours === 24 
        ? volumeNh 
