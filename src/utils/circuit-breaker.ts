...
export class CircuitBreaker {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (state.state === 'open') {
      ...
      if (timeSinceLastFailure >= this.options.recoveryTimeout) {
        ...
        const nextRetryTime = (state.lastFailureTime || 0) + this.options.recoveryTimeout;
    ...
    if (state.state === 'half-open' && state.consecutiveSuccesses >= this.options.halfOpenMaxAttempts) {
  ...
  async getState(): Promise<CircuitBreakerState> {
    try {
      ...
      if (cached) {
        return JSON.parse(cached) as CircuitBreakerState;
      }
  ...
  private async recordFailure(): Promise<void> {
    try {
      ...
      if (state.state !== 'open' && recentFailures >= this.options.failureThreshold) {
  ...
  private async getRecentFailures(): Promise<number> {
    try {
      ...
      await redis.getClient().zremrangebyscore(failureKey, '-inf', windowStart.toString());
      const count = await redis.getClient().zcount(failureKey, windowStart.toString(), '+inf');
  ...
  async getStats(): Promise<{
    ...
    try {
      const failures = await redis.getClient().zcount(
        `${this.metricsKey}:failures`,
        windowStart.toString(),
        '+inf'
      );
      
      const successes = await redis.getClient().zcount(
        `${this.metricsKey}:successes`,
        windowStart.toString(),
        '+inf'
      );
